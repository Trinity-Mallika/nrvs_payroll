...
$pagename = "biomatric_manual_att_report.php";
$title = "Biometric vs Manual Attendance Difference";
$tblname = "attendance_entry";
$tblpkey = "attendance_id";
$module = "Biometric vs Manual Attendance Difference";
$submodule = "Biometric vs Manual Attendance Difference";
$btn_name = "Save";
                                        ...
                                        <table id="buttons-datatables" class="display table table-sm table-bordered"
                                            ...
                                            <thead>
                                                <tr class="table-primary">
                                                    ...
                                                    <th>Department</th>
                                                    <th>Basic Salary</th>
                                                    <th>Total Attendance Record</th>
                                                    <th>Machine Punch In</th>
                                                    <th>Machine Punch Out</th>
                                            ...
                                            <tbody>
                                                <?php
                                                    ...
                                                    $res = $obj->executequery("SELECT
                                                            ...
                                                            em.basic_salary,
                                                            ae.emp_id,
                                                            COUNT(ae.emp_id) as total_att,

                                                            SUM(CASE
                                                                WHEN ae.entry_type = 'machine' AND ae.in_status = 'IN'
                                                                THEN 1 ELSE 0 END) AS machine_in,

                                                            SUM(CASE
                                                                WHEN ae.entry_type_out = 'machine' AND ae.out_status = 'OUT'
                                                                THEN 1 ELSE 0 END) AS machine_out,

                                                            SUM(CASE
                                                                WHEN ae.entry_type = 'manual' AND ae.in_status = 'IN'
                                                                THEN 1 ELSE 0 END) AS manual_in,

                                                            SUM(CASE
                                                                WHEN ae.entry_type_out = 'manual' AND ae.out_status = 'OUT'
                                                                THEN 1 ELSE 0 END) AS manual_out
                                                        ...
                                                        ");
                                                    foreach ($res as $row) {
                                                        $total_machine_in += $row['machine_in'];
                                                        $total_machine_out += $row['machine_out'];
                                                        $total_manual_in += $row['manual_in'];
                                                        $total_manual_out += $row['manual_out'];
                                                        $gt_basic_salary += $row['basic_salary'];
                                                        $total_attendance += $row['total_att'];
                                                    ?>
                                                <tr>
                                                    <td>
                                                        <?= $slno++ ?>
                                                    ...
                                                    <td>
                                                        <a target='_blank'
                                                            href='employee_view.php?emp_id=<?= $row['emp_id']  ?>&month=<?= $month ?>&year=<?= $year ?>&submit=Search'
                                                    ...
                                                    <td class="text-end"><?= $row['total_att'] ?></td>
                                                    <td class="text-end"><?= $row['machine_in'] ?></td>
                                                    <td class="text-end"><?= $row['machine_out'] ?></td>
                                                    <td class="text-end"><?= $row['manual_in'] ?></td>
                                                    <td class="text-end"><?= $row['manual_out'] ?></td>
                                                </tr>
                                                <?php } ?>
                                            </tbody>
                                            <tfoot>
                                                <tr style="font-weight:bold;background:#f1f1f1;">
                                                    ...
                                                    <td class="text-end"><?= $total_attendance ?></td>

                                                    <td class="text-end"><?= $total_machine_in ?></td>

                                                    <td class="text-end"><?= $total_machine_out ?></td>

                                                    <td class="text-end"><?= $total_manual_in ?></td>

                                                    <td class="text-end"><?= $total_manual_out ?></td>
                                                </tr>
                                            </tfoot>
                                        </table>
