...
<head>
    ...
    <link rel="stylesheet" href="https://unpkg.com/filepond-plugin-image-edit/dist/filepond-plugin-image-edit.min.css">
...
<body>
                                        ...
                                        <div class="form-step form-step-active">
                                            ...
                                            <div class="row g-3">
                                                ...
                                                <div class="col-md-2">
                                                    <label class="form-label">Is Active<span
                                                            class="text-danger fw-bold">*</span> </label>
                                                    <select class="form-select form-select-sm chosen-select" name="is_active"
                                                        id="is_active">
                                                        <option value="">Select</option>
                                                        <option value="1">YES</option>
                                                        <option value="0">NO</option>
                                                ...
                                                <div class="col-md-3">
                                                    <label class="form-label">Employee Signature <span
                                                            class="text-danger fw-bold">(Please Remove Image background then Upload)</span><a href="https://www.remove.bg/" target="_blank">  click here</a></label>
                                                    <input type="file" name="emp_sign" id="emp_sign">
                                                </div>
                                            ...
                                            <div class="mt-4 text-end">
                                                <button type="button" class="btn btn-primary btn-next"
                                                    data-validate="emp_code,biomatric_id,first_name,father_name,department_id,designation_id,date_of_joining,basic_salary,dob,mobile_no,aadhar_no,shift_id,is_pf,is_esic,allow_weekly_off,is_perform_incen,is_active,reporting_manager,present_address">
                                                    Next
                                                    →</button>
                                            </div>
                                        </div>
 <!-- data-validate="emp_code,biomatric_id,first_name,father_name,department_id,designation_id,date_of_joining,basic_salary,emp_category,dob,blood_group,mobile_no,aadhar_no,shift_id,is_pf,is_esic,allow_weekly_off,is_perform_incen,reporting_manager,marital_status,present_address,permanent_address,gender,profile_image" -->
    ...
    function validateForm() {
        const steps = [{
                ...
                fields: ['emp_code', 'biomatric_id', 'first_name', 'father_name', 'department_id', 'designation_id',
                    'date_of_joining', 'basic_salary', 'dob', 'mobile_no', 'aadhar_no', 'shift_id', 'is_pf','is_esic', 'allow_weekly_off','is_perform_incen','is_active','reporting_manager','present_address' 
                ]
            },
    ...
    var currentEditorRef = null;

    var cropperEditor = {
        open: function(file, instructions) {
            currentEditorRef = this;
            ...
            reader.onload = function(e) {
                ...
                currentCropper = new Cropper(image, {
                    ...
                    ready: function() {
                        if (instructions && instructions.crop && instructions.crop.aspectRatio) {
                            currentCropper.setAspectRatio(instructions.crop.aspectRatio);
                        }
                    }
                });
            ...
            var onConfirm = function() {
                ...
                var canvasData = currentCropper.getCanvasData();
                var cropData = currentCropper.getData(true);
                var naturalW = canvasData.naturalWidth;
                var naturalH = canvasData.naturalHeight;

                var cx = (cropData.x + cropData.width / 2) / naturalW;
                var cy = (cropData.y + cropData.height / 2) / naturalH;
                var aspect = cropData.width / cropData.height;

                currentCropper.destroy();
                currentCropper = null;
                currentModal.hide();

                if (currentEditorRef && currentEditorRef.onconfirm) {
                    currentEditorRef.onconfirm({
                        data: {
                            crop: {
                                center: { x: cx, y: cy },
            ...
            document.getElementById('cropConfirm').onclick = onConfirm;
    ...
    var signPond = FilePond.create(document.querySelector('#emp_sign'), {
        ...
        maxFileSize: '2MB',
        allowImageEdit: true,
        imageEditEditor: cropperEditor,
        imageEditInstantEdit: true,
        imageEditAllowEdit: true,
        labelIdle: 'Drag & Drop signature or <span class="filepond--label-action">Browse</span>'
    });

    <?php if ($emp_sign != "" && $keyvalue > 0) { ?>
    signPond.addFile('<?php echo $imgpath1 . $emp_sign; ?>').then(function() {}).catch(function(err) { console.log(err); });
    <?php } ?>
    </script>
