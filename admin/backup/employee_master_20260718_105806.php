...
<body>
 ...
 <!-- data-validate="emp_code,biomatric_id,first_name,father_name,department_id,designation_id,date_of_joining,basic_salary,emp_category,dob,blood_group,mobile_no,aadhar_no,shift_id,is_pf,is_esic,allow_weekly_off,is_perform_incen,reporting_manager,marital_status,present_address,permanent_address,gender,profile_image" -->
    ...
    signPond.on('addfile', function(err, file) {
        if (err || isCropping) return;
        isCropping = true;
        ...
        document.getElementById('cropConfirm').onclick = function() {
            ...
            canvas.toBlob(function(blob) {
                ...
                cropper = null;
                modal.hide();
                var croppedFile = new File([blob], file.file.name, { type: 'image/png' });

                var fileId = file.id;
                signPond.removeFile(fileId).then(function() {
                    return signPond.addFile(croppedFile);
                }).then(function() {
    ...
    isCropping = true;
    signPond.addFile('<?php echo $imgpath1 . $emp_sign; ?>').then(function() {
        isCropping = false;
    }).catch(function(err) { isCropping = false; console.log(err); });
    <?php } ?>
    </script>
