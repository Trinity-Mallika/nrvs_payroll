...
$mode = $obj->test_input($_POST['mode']??'New');
if ($type == 'family') {
...
<tr>
    ...
    <td class="text-center">
        <div class="d-flex align-items-center wt">
            <div class="flex-grow-1 ms-2 name">
                <label class="switch">
                    <input type="checkbox" <?php if ($row['is_nominee'] == 1) echo "checked"; ?>>
                    <span class="slider round" onclick="change_nominee('<?php echo $row['family_detail_id']; ?>');">
                    </span>
                </label>
            </div>
        </div>
    ...
    <td>
        <?php if($mode!='view'){ ?>
        <button type="button" class="btn btn-primary btn-sm" onclick="edit_family(
        ...
        <button type="button" class="btn btn-danger btn-sm btn-label right ms-auto nexttab nexttab"
            ...
            onclick="delete_family('<?= $row['family_detail_id']; ?>','<?= $row['aadhar_card']; ?>');"><i
...
<tr>
    ...
    <td>
        <?php if($mode!='view'){ ?>
        <button type="button" class="btn btn-primary btn-sm" onclick="edit_education(
            '<?= $row['education_id']; ?>',
...
<tr>
    ...
    <td>
         <?php if($mode!='view'){ ?>
        <button type="button" class="btn btn-success btn-sm" onclick="edit_document(
                '<?= $row['emp_doc_id']; ?>',
                '<?= $row['doc_id']; ?>',
                '<?= $row['doc_expiry_date']; ?>',
        ...
        <button type="button" class="btn btn-danger btn-sm btn-label right ms-auto nexttab nexttab"
            data-nexttab="pills-experience-tab"
            onclick="delete_document('<?= $row['emp_doc_id']; ?>','<?= $row['doc_file']; ?>');"><i
                class="ri-delete-bin-5-line label-icon align-middle fs-14 ms-2"></i>Del</button>
                 <?php } ?>
    </td>
</tr>
...
<tr>
    ...
    <td>
<?php if($mode!='view'){ ?>
        <button type="button" class="btn btn-success btn-sm" onclick="edit_language(
                '<?= $row['emp_language_id']; ?>',
...
<tr>
    ...
    <td class="text-center">
        <div class="d-flex align-items-center wt">
            <div class="flex-grow-1 ms-2 name">
                <label class="switch">
                    <input type="checkbox" <?php if ($row['is_active'] == 1) echo "checked"; ?>>
    ...
    <td>
<?php if($mode!='view'){ ?>
        <button type="button" class="btn btn-success btn-sm" onclick="edit_bank(
                '<?= $row['emp_bank_id']; ?>',
                '<?= $row['bank_id']; ?>',
