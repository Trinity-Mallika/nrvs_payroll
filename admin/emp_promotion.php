...
if (isset($_POST['promote_emp_id'])) {
    ...
    $prev_salary = $obj->test_input($_POST['prev_salary']);
    $promote_amt = $obj->test_input($_POST['promote_amt']);
    $basic_salary = $obj->test_input($_POST['new_salary']);
    $modal_type = $obj->test_input($_POST['modal_type']);
...
<body>
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td>
                                                                ...
                                                                <span
                                                                    ...
                                                                    <?= $key['status'] == 0
                                                                                                                                                                                                                    ? "style='cursor:pointer;' onclick=\"updatePromotionStatus('{$key['emp_promotion_id']}', '{$key['emp_id']}', '{$key['department_id']}', '{$key['designation_id']}', '{$key['basic_salary']}', '{$key['status']}', '{$key['effected_month']}', '{$key['effected_year']}')\""
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4 mb-3">
                                ...
                                <input type="text" name="emp_name" class="form-control form-control-sm"
                                    value="<?= $first_name . " " . $last_name . " - (" . $emp_code . ")" ?> " readonly>
                                <input type="hidden" id="modal_emp_id" value="<?= $emp_id ?>"
                                    class="form-control form-control-sm">
                                <input type="hidden" id="modal_emp_code" value="<?= $emp_code ?>"
                                    class="form-control form-control-sm">
                            </div>

                            <div class="col-md-4 mb-3">
                                <label>Type <span class="text-danger fw-bold">*</span> </label>
                                <select id="modal_type" class="form-select form-select-sm chosen-select">
                            ...
                            <div class="col-md-4 mb-3">
                                <label>New Department <span class="text-danger fw-bold">*</span> </label>
                                <select id="modal_department_id" class="form-select form-select-sm chosen-select"
                            ...
                            <div class="col-md-4 mb-3">
                                <label>New Designation <span class="text-danger fw-bold">*</span> </label>
                                <select id="modal_designation_id" class="form-select form-select-sm chosen-select">
                                    <option value="">Select Designation</option>
                                    <?php $res = $obj->executequery("Select * from designation_master where unit_id='$unitid' and designation_id='$designation_id' order by designation asc");
                                    foreach ($res as $key) { ?>
                                ...
                                <script>
                                document.getElementById('modal_designation_id').value = '<?= $designation_id ?>'
                                </script>
                            </div>
                            <!-- Last Working Date -->
                            <div class="col-md-4 mb-3">
                                <label>Previous Salary<span class="text-danger fw-bold">*</span> </label>
                                <input type="text" id="modal_prev_salary" class="form-control form-control-sm"
                                    value="<?= $basic_salary ?>" readonly onkeypress="numberOnly(event)">
                            </div>


                            <div class="col-md-4 mb-3">
                                <label>Promote Amount<span class="text-danger fw-bold">*</span> </label>
                                <input type="text" id="modal_pramote_amt" class="form-control form-control-sm"
                                    onkeypress="numberOnly(event)" oninput="calculateNewSalary();"
                                    placeholder="Enter Promote Amt">
                            </div>

                            <div class="col-md-4 mb-3">
                                <label>New Salary<span class="text-danger fw-bold">*</span> </label>
                                <input type="text" id="modal_new_salary" class="form-control form-control-sm"
                                    value="<?= $basic_salary ?>" onkeypress="numberOnly(event)" readonly>
                            </div>

                            <div class="col-md-4 mb-3">
                                <label>Promotion Date<span class="text-danger fw-bold">*</span> </label>
                                <input type="date" id="modal_promotion_date" class="form-control form-control-sm"
                                    value="<?= date('Y-m-d'); ?>" max="<?= date('Y-m-d'); ?>">
                            </div>

                            <div class="col-md-4 mb-3">
                                <label for="month" class="form-label">Effected Month<span
                                        class="text-danger fw-bold">*</span></label>
                                <select class="form-select chosen-select" id="effected_month">
                            ...
                            <div class="col-md-4 mb-3">
                                <label for="year" class="form-label">Year<span
                                        class="text-danger fw-bold">*</span></label>
                                <select class="form-select chosen-select" id="effected_year">
                                    <option value="">Select</option>
                                    <?php
                                    $startYear = 2025;
                                    $endYear = 2100;
    ...
    function saveTransfer() {
        ...
        const prev_salary = $('#modal_prev_salary').val();
        const promote_amt = $('#modal_pramote_amt').val();
        const new_salary = $('#modal_new_salary').val();
        const modal_remark = $('#modal_remark').val();
        const effected_month = $('#effected_month').val();
        const effected_year = $('#effected_year').val();
        const modal_type = $('#modal_type').val();
        const modal_type_text = $('#modal_type option:selected').text();
        ...
        if (emp_id == "" || department_id == "" || promotion_date == "" || new_salary == '' || promote_amt == '' ||
            effected_month == '' || effected_year == '') {
            Swal.fire({
                icon: 'warning',
                title: 'Required',
        ...
        Swal.fire({
            title: 'Are you sure?',
            html: `
        Are you sure you want to ${modal_type_text} this employee to 
    ...
    function calculateNewSalary() {
        let prevSalary = parseFloat(document.getElementById('modal_prev_salary').value) || 0;
        let promoteAmt = parseFloat(document.getElementById('modal_pramote_amt').value) || 0;

        let newSalary = prevSalary + promoteAmt;

        document.getElementById('modal_new_salary').value = newSalary;
    }
    </script>
</body>
