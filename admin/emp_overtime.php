...
$title = "Employee Extra Off";
$tblname = "emp_monthly_leave";
$tblpkey = "month_leave_id";
$module = "Employee Extra Off";
$submodule = "Employee Extra Off List";
$btn_name = "Save";
...
if (isset($_POST['submit'])) {
    ...
    $no_of_overtime = $obj->test_input($_POST['no_of_overtime']);
...
if (isset($_GET[$tblpkey])) {
    ...
    $month = $sqledit['month'];
    $year = $sqledit['year'];
    $no_of_overtime = $sqledit['no_of_overtime'];
} else {
    $emp_id = "";
    $month = "";
    $no_of_overtime = "";
        ...
        if ($fileType === 'xlsx') {
            if ($xlsx = SimpleXLSX::parse($_FILES['file_upload']['tmp_name'])) {
                foreach ($xlsx->rows() as $k => $data) {
                    ...
                    list($emp_code, $no_of_overtime) = $data;
                    ...
                    $form_data1 = array(
                        ...
                        "month" => $file_month,
                        "year" => $file_year,
                        "total_leave" => $no_of_overtime,
                        "remining_leave" => $no_of_overtime,
                        "leave_type" => 'eoff',
                        "createdby"   => $loginid,
                        "ipaddress"   => $ipaddress,
                        "sessionid"   => $sessionid,
