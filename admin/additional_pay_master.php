...
if (isset($_POST['submit'])) {
    ...
    $where = array('emp_id' => $emp_id, 'unit_id' => $unitid, 'year' => $year, 'month' => $month);
...
if (isset($_GET[$tblpkey])) {
    ...
    $month = $sqledit['month'];
    $year = $sqledit['year'];
    $additional_payment = $sqledit['additional_payment'];
} else {
    $emp_id = "";
    $month = "";
    $additional_payment = "";
        ...
        if ($fileType === 'xlsx') {
            if ($xlsx = SimpleXLSX::parse($_FILES['file_upload']['tmp_name'])) {
                foreach ($xlsx->rows() as $k => $data) {
                    ...
                    list($emp_code, $additional_payment) = $data;
                    ...
                    $where = array('emp_id' => $emp_id, 'unit_id' => $unitid, 'year' => $file_year, 'month' => $file_month);
                    // print_r($where);
                    // die;
                            ...
                            <div class="card" id="customerList">
                                ...
                                <div class="card-body">
                                    <div class="row">
                                        ...
                                        <div class="col-lg-6 mb-3">
                                            <label for="additional_payment" class="form-label">Additional Payment <span class="text-danger" id="net_payment"></span></label>
                                            <input type="text" id="additional_payment" name="additional_payment" class="form-control form-control-sm" placeholder="Enter Additional Payment" value="<?= $additional_payment ?>" autocomplete="off" onkeypress="numberOnly(event);" />
                                        </div>
                                        ...
                                        if ($chkadd == 1) {  ?>
                                            <div class="col-lg-4 mb-3">
                                                ...
                                                <input type="submit" name="submit" class="btn btn-sm btn-primary add-btn" value="<?php echo $btn_name ?>" onClick="return checkinputmaster('emp_id,month,year,additional_payment')">
