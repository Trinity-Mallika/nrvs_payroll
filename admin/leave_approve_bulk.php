...
$current_time = date('H:i:s');
 $is_all_leave_add = $obj->getvalfield("unit_master", "add_leave", "unit_id='$unitid'")??'0';
if(isset($_POST['bulkApprove'])){
    ...
    $onDutyIds = array_map('intval', $ids);
    $onDutyIdsStr = implode(',', $onDutyIds);

    $allDetails = $obj->executequery("
        SELECT
            lad.*, 
            odm.emp_id, 
        ...
        LEFT JOIN on_duty_master odm
            ON odm.on_duty_id = lad.on_duty_id

        LEFT JOIN employee_master em
            ON em.emp_id = odm.emp_id

        LEFT JOIN department_master dm
            ON em.department_id = dm.department_id

        WHERE lad.on_duty_id IN ($onDutyIdsStr)

        ORDER BY lad.on_duty_id, lad.date
    ");

    $detailsMap = [];
    foreach ($allDetails as $row) {
        $detailsMap[$row['on_duty_id']][] = $row;
    }

    foreach($ids as $on_duty_id){
        $details = $detailsMap[$on_duty_id] ?? [];
        if (empty($details)) {
            continue;
        }
        // Employee data is already available
        $emp_data = $details[0];
        ...
        $is_allow_c_off =  $emp_data['c_off_check']??'0'; 
        $allow_earn_leave_carry = $emp_data['earn_leave_check']??'0';  
      
        $empIds = [];
        
        $empIds[] = $emp_id;
        $empIdsStr = implode(',', array_unique($empIds)); 
        ...
        foreach($details as $row){
            ...
            if ($status == 1) {
                ...
                $cacheMonthYear = $month . '_' . $year;

                /************ EARNING LEAVE ************/
                if (!isset($earningCache[$cacheMonthYear])) {

                    $earningLeaveRows = $obj->earningLeaveRows($sessionid,$month,$year);

                    foreach($earningLeaveRows as $r){
                        $usedEarnMap[$cacheMonthYear][$r['emp_id']] = $r['used_leave'];
                    }

                    $earningUploadRows = $obj->earningUploadRows($sessionid,$month,$year);

                    foreach($earningUploadRows as $r){
                        $earningUploadMap[$cacheMonthYear][$r['emp_id']] = $r['total_leave'];
                    }

                    $earningCache[$cacheMonthYear] = true;
                }
                /************ COFF ************/
                if (!isset($coffCache[$cacheMonthYear])) {

                    $coffUsedRows = $obj->getEmpUsedCoff($empIdsStr,$sessionid,$month,$year);

                    foreach($coffUsedRows as $r){
                        $usedCoffMap[$cacheMonthYear][$r['emp_id']] = $r['used_coff'];
                    }

                    $coffUploadRows = $obj->getEmpUploadedCoff($empIdsStr,$sessionid,$month,$year);

                    foreach($coffUploadRows as $r){
                        $coffUploadMap[$cacheMonthYear][$r['emp_id']] = $r['total_leave'];
                    }

                    $coffCache[$cacheMonthYear] = true;
                }
                /************ EXTRA OFF ************/
                if (!isset($extraOffCache[$year])) {

                    $rows = $obj->extraOffUpload($sessionid,$year);

                    foreach($rows as $r){
                        $uploadArr[$year][$r['emp_id']][(int)$r['month']] =
                            (float)$r['total_leave'];
                    }

                    $rows = $obj->extraOffUsed($sessionid,$year);

                    foreach($rows as $r){
                        $usedArr[$year][$r['emp_id']][(int)$r['month']] =
                            (float)$r['total_used'];
                    }

                    $extraOffCache[$year] = true;
                }
                ...
                if ($leave_type == 'EL') {
                    ...
                    $leave_balance =
                        ($earningUploadMap[$cacheMonthYear][$emp_id] ?? 0)
                        -
                        ($usedEarnMap[$cacheMonthYear][$emp_id] ?? 0);
                ...
                $leave_balance = $extraOff['balance']??0;

                } elseif ($leave_type == 'CO') {

                    $leave_balance =
                        ($coffUploadMap[$cacheMonthYear][$emp_id] ?? 0)
                        -
                        ($usedCoffMap[$cacheMonthYear][$emp_id] ?? 0);
