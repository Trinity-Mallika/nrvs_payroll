...
$pagename = "pf_esic_upload.php";
$title = "PF ESIC Excel Upload";
$tblname = "salary_structure";
$tblpkey = "salary_struc_id";
$module = "PF ESIC Excel Upload";
$submodule = "PF ESIC Excel Upload List";
$btn_name = "Save";
        ...
        if ($fileType === 'xlsx') {
            if ($xlsx = SimpleXLSX::parse($_FILES['file_upload']['tmp_name'])) {
                foreach ($xlsx->rows() as $k => $data) {
                    ...
                    list($emp_code, $is_pf, $is_esic) = $data;
                    ...
                    // Yes / No convert
                    $is_pf   = ($is_pf == 'Yes') ? '1' : '0';
                    $is_esic = ($is_esic == 'Yes') ? '1' : '0';

                    // Update record
                    $obj->update_record(
                        'employee_master',
                        ['emp_id' => $emp_id, 'unit_id' => $unitid],
                        ['is_pf' => $is_pf, 'is_esic' => $is_esic]
                    );
