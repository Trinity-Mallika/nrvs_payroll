...
// $getLeave = $obj->getLeave('28', "2", "2026");
// print_r($getLeave);
// die;
...
if (isset($_POST['month'], $_POST['year'])) {
    ...
    // $employees = $obj->executequery("SELECT em.*, em.emp_id as main_emp_id, SUM(CASE WHEN ae.attendance_status = 'Present' THEN 1 WHEN ae.attendance_status = 'Half Day' THEN 0.5 ELSE 0 END ) AS total_working_day FROM employee_master em  INNER JOIN attendance_entry ae ON ae.emp_id = em.emp_id  AND ae.month='$month' AND ae.year='$year' WHERE em.unit_id = '$unitid' $crit group by em.emp_id ORDER BY em.emp_id DESC");
    ...
    if (count($employees) > 0) {
        ...
        foreach ($employees as $emp) {
            ...
            $increment     = 0; // if any logic
            $revisedSalary = roundVal($presentSalary + $increment);
            $daysWorked    = $total_working_day ?: $daysInMonth;
            ...
            $totalAllowedDays = $total_working_day + $total_leave;
            ...
            $basicDA   = roundVal($basicRate / $totalDaysInMonth * $daysWorked);

            $hra       = roundVal($basicDA * $slab['hra_percent'] / 100);
            $medical   = roundVal($slab['medical_allow'] / $totalDaysInMonth * $daysWorked);
            $convey    = roundVal($slab['conve_allow'] / $totalDaysInMonth * $daysWorked);

            $perDaySalary = roundVal($revisedSalary / $totalDaysInMonth * $daysWorked);
            $special      = $perDaySalary - ($basicDA + $hra + $medical + $convey);

            /* 🔹 PF / ESIC */
            // $pf_emp  = ($basicRate <= 15000) ? roundVal($basicDA * $slab['pf_per'] / 100) : 0;
            // $esic_emp = ($basicRate <= 21000) ? roundVal($basicDA * $slab['esic_per'] / 100) : 0;

            // $pf_employer  = ($basicRate <= 15000) ? roundVal($basicDA * $slab['pf_emp_per'] / 100) : 0;
            // $esic_employer = ($basicRate <= 21000) ? roundVal($basicDA * $slab['esic_emp_per'] / 100) : 0;
            ...
            if ($is_pf == 1) {
                if ($revisedSalary > 15000) {
                    ...
                    $pf_paid_basic = round($pf_rate / $totalDaysInMonth * $daysWorked);
                }
                            ...
                            <div class="card bg-body">
                                <div class="card-header border-bottom-dashed">
                                    <div class="row g-4 align-items-center">
                                        <div class="col-sm">
                                            <div>
                                                <h5 class="card-title mb-0 text-primary">Monthly Payment
                                                </h5>
                                                <br>
                                                <h6 class="card-title mb-0 text-primary">
                                                    Total C-Off - <span id="total_c_off_text"> <?= ($keyvalue > 0) ? $total_c_off : $three_month_leave; ?></span><br>
                                                    Used C-Off - <?php if ($keyvalue > 0) { ?>
                                                        <?= $c_off_leave ?>
                                                    <?php } else { ?>
                                                        <span id="used_c_off_text">0</span>
                                                    <?php } ?><br>
                                                    Remaining C-Off - <?php if ($keyvalue > 0) { ?>
                                                        <?= $total_c_off - $c_off_leave ?>
                                                    <?php } else { ?>
                                                        <span id="remaining_c_off_text">0</span>
                                                    <?php } ?>
                                                </h6>
                                            </div>
                                        </div>
                                ...
                                <div class="card-body">
                                    <form method="post" id="salaryForm">
                                        <div class="row">
                                            ...
                                            <div class="col-lg-2 col-12 mb-3">
                                                <label for="">Total Payable Salary </label>
                                            ...
                                            <div class="col-lg-2 col-12 text-center mt-4">
                                                <input type="submit" name="submit" class="btn btn-primary add-btn" value="<?= $btn_name ?>">
                                            </div>
                                        </div>
                                    </form>
                                </div>
