...
<body>
    <section class="top-sec ">
        ...
        <div class="container">
            <div class="row g-2 mb-3 align-items-end">
                <div class="card">
                    <div class="col-12">
                        ...
                        <select class="form-control chosen-select" name="select_emp_id" id="select_emp_id"
                            onchange="get_url(this.value)">
                            <option value="">Select Employee</option>
                            <?php 
                            $res = $obj->executequery("SELECT * FROM employee_master WHERE unit_id = '$unitid' AND (resign_status != '1' OR (resign_status = '1' AND last_working_date >= CURDATE())) and reporting_manager = '$_SESSION[emp_id]'  ORDER BY first_name ASC");
                            foreach ($res as $key) { ?>
                            <option value="<?= $key['emp_id']; ?>">
                                <?= $key['emp_code']; ?>-<?= ucfirst($key['first_name'] ?? ''); ?></option>
                            <?php } ?>
                        </select>
                        <script>
                        document.getElementById('select_emp_id').value =
                            '<?= $emp_id; ?>';
                        </script>
                    </div>
                </div>
            ...
            <h6 class="text-center text-blue mt-3 fs-5"><?=$emp_code .' - '. $emp_name?></h6>
    ...
    function get_url(emp_id) {
        location = 'emp_att.php?emp_id=' + emp_id;
    }
    </script>
