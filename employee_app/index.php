...
// Check if the string contains "4.0 Mobile Safari"
if (strpos($userAgent, "4.0 Mobile Safari") !== false) {
  $is_android = 1;
  $status = "link Url";
  $status_new = " Android User";
} else {
  $status_new = "IOS User";
  $status = " App";
}
...
if (isset($_COOKIE['NRVSGroup_mobile_no']))
  $mobile_no = $obj->test_input($_COOKIE['NRVSGroup_mobile_no']);
else
  $mobile_no = "";



if (isset($_COOKIE['NRVSGroup_password']))
  $password = $obj->test_input($_COOKIE['NRVSGroup_password']);
else
  $password = "";
/*if ($mobile_no != "" && $password != "") {
  $is_exist = $obj->login_method_app("m_employee", $mobile_no, $password);
  if ($is_exist > 0) {
...
if (isset($_POST['submit'])) {
  ...
  //$is_iphone =  $obj->getvalfield("employee_master", "is_iphone", "mob_no='$mobile_no' and emp_password='$password'");
  ...
  if ($username != "" && $password != "") {
    ...
    if ($count > 0) {
      ...
      setcookie("NRVSGroup_mobile_no", $username, time() + (86400 * 7), "/");
      setcookie("NRVSGroup_password", $password, time() + (86400 * 7), "/");
            ...
            <div class="row">
              <div class="input-field col s12 m6 l4 offset-m3 offset-l4">
                <input id="username" type="number" name="username" onkeypress="validate(event)" maxlength="10"
                  value="<?php echo $mobile_no; ?>" autocomplete="off" onkeyup="allowNumbersOnly(this)" type="text"
                  maxlength="10">
